...
void KEY_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_9;       //PC15
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;    //设置成上拉输入，平时为高，按下时低电平拉低
 	GPIO_Init(GPIOB, &GPIO_InitStructure);           //初始化GPIOC15
    
    GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_11;       //PC15
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;    //设置成上拉输入，平时为高，按下时低电平拉低
 	GPIO_Init(GPIOB, &GPIO_InitStructure);           //初始化GPIOC15
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;				    //触发引脚-->PC14 端口配置,由于是需要一个高电平脉冲，所以要设置为推挽式输出
