...
void DS18B20_Rst(void)
{
	...
	delay_us1(20);   //从机接收到主机的复位信号后，会在15~60us后给主机发一个存在脉冲
}
...
u8 DS18B20_Presence(void)
{
	 ...
	 */
	while( DS18B20_DATA_IN() && pulse_time<140 )
	{
		pulse_time++;
		delay_us1(1);
	}	

	if( pulse_time >=140 )  //经过140us后，存在脉冲都还没有到来
		return 1;             //读取失败
	else                 //经过140us后，存在脉冲到来
		pulse_time = 0;    //清零计时变量
...
void DS18B20_Write_Byte(u8 dat)
{
	...
	for( i=0; i<8; i++ )
	{
		...
		{			
			DS18B20_DATA_OUT(LOW);
			delay_us1(12);   //1us < 这个延时 < 15us
			
			DS18B20_DATA_OUT(HIGH);
			delay_us1(78);    //58us+8us>60us
		}		
		else
		{			
			DS18B20_DATA_OUT(LOW);  
